...
## Because Github can share data, lots of people can see your information and change it, so it can be leaked.
...
# The output value is null, that is, no code block is displayed but the final result is displayed.
```
...
data1 <- data %>% filter(!(is.na(activity))) 
data1
```
...
data1 %>% select("species", "treatment", "animal_id", "activity")
...
data2 <- data1 %>% group_by(species, treatment) %>% summarise(mean_activity = mean(activity),  standerr_activity = std.error(activity), mean_sl = mean(sl),  standerr_sl = std.error(sl),)
data2
# Use flextable to render the summary table in a tidy format
flextable(
  data2,
  col_keys = names(data2),
  cwidth = 0.75,
  cheight = 0.25,
  defaults = list(),
...
## Red highlights indicate that they have not been added to version control and has just been cloned locally. While the green highlights indicate that they have been added to version control, is a new part but has not yet been submitted.
```
...
ggplot(data = data2, aes(x = treatment, y = mean_activity, color = species)) + geom_boxplot()
```
...
```{r}
picture1 <- readPNG("./pics/acantho.png")
picture2 <- readPNG("./pics/lemon.png")
picture3 <- readPNG("./pics/ambon.png")
picture4 <- readPNG("./pics/humbug.png")
picture5 <- readPNG("./pics/chromis.png")
picture6 <- readPNG("./pics/whitedams.png")
ggplot1 <- ggplot(data = data2, mapping = aes(x = species, y = mean_activity, color = treatment)) +
  geom_boxplot() + annotation_raster(picture1, 0.7, 1.3, 20, 25) + annotation_raster(picture2, 1.7, 2.3, 20, 25) + annotation_raster(picture3, 2.7, 3.3, 20, 25) + annotation_raster(picture4, 3.7, 4.3, 20, 25) + annotation_raster(picture5, 4.7, 5.3, 20, 25) + annotation_raster(picture6, 5.7, 6.3, 20, 25)
ggplot1
```
...
ggplot2 = ggplot(data = data2, aes(x = treatment, y = mean_activity, color = species)) + geom_boxplot()
ggplot2

ggsave("plot.png",ggplot2 )
```
...
```{r}
## I can see the saved image and save path before adding it. But once in ".gitignore", there is no way to push the image or directory to the remote repository. The purpose of the ".gitignore" file is to specify that Git does not track the file.
```
...
```{r, answer6}
## I don't want it to track because I saved it as an image, but even though other people in the cloud can track my save path and results, they can't change the image. Tracking is best if it can be changed, but this one doesn't have to be.
```
...
data3 <- data2 %>% filter(species=="chromis")
data4 <- data2 %>% filter(species=="lemon")
data5 <- data2 %>% filter(species=="acantho")
mydata <- rbind(data3,data4,data5)
mydata

ggplot(data = mydata, aes(x = species, y = mean_activity, color = treatment)) +
  geom_boxplot()+annotation_raster(picture1,0.7,1.3,19,21)+annotation_raster(picture3,1.7,2.3,19,21)+annotation_raster(picture5,2.7,3.3,19,21)
...
```{r}
data6 <- data1 %>% select("species", "treatment", "animal_id", "activity")
data7 <- data6[which(data6$species=='chromis'),]
data8 <- data6[which(data6$species=='ambon'),]
data9 <- data6[which(data6$species=='humbug'),]
data10 <- data6[which(data6$species=='lemon'),]
data11 <- data6[which(data6$species=='acantho'),]
data12 <- data6[which(data6$species=='whitedams'),]


data71<-t.test(data7$activity ~ data7$treatment)
data71
data81 <- t.test(data8$activity ~ data8$treatment)
data81
data91 <- t.test(data9$activity ~ data9$treatment)
data91
data101 <- t.test(data10$activity ~ data10$treatment)
data101
data111 <- t.test(data11$activity ~ data11$treatment)
data111
data121 <- t.test(data12$activity ~ data12$treatment)
data121
data_statistic <- c(data71$statistic, data81$statistic, data91$statistic, data101$statistic, data111$statistic, data121$statistic)
data_pvalue <- c(data71$p.value, data81$p.value, data91$p.value, data101$p.value, data111$p.value, data121$p.value)
data_df <- c(data71$parameter, data81$parameter, data91$parameter, data101$parameter, data111$parameter, data121$parameter)
```
...
species <- c('chromis','ambon','humbug','lemon','acantho','whitedams')
tibble(species,data_statistic,data_pvalue,data_df)
```
...
I am most interested in "humbug", which has an average of 40 for the CO2 group treatment and 36 for the control group treatment, so the difference in mean is 4. The 95% confidence intervals for their differences were (-0.29, 8.4). Assuming no relationship between control and CO2 treatment, the above analysis yielded t=1.85, p=0.067, df=137. Because the p-value is greater than 0.05, it indicates no significant difference. According to the t value and df, the table shows that there is no significant difference between the control and CO2 treatments. So we accept our null hypothesis. I was more interested in the Humbug species because none of them had a p-value greater than 0.05, so it was more general.
